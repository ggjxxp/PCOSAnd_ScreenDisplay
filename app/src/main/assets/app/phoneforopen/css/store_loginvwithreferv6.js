	...
	$('#login_form').submit(function(){
		...
		if($('#login_account').val()==''){
			...
			$.post(login_check,$("#login_form").serialize(),function(result){
				...
				if(result){
					if(result.error == 0){
						window.localStorage.setItem("cyberwin_client_session", result.cwpd_session);
						//alert(result.cwpd_session);
						notice(result.msg,'ok');


						//alert(result.referer);
						var cwpd_url=result.referer.replace("&amp;","&");

						//notice(result.referer,'error');
						//return;
						setTimeout(function(){
							//window.parent.location = store_index;
							if(result.referer.length>5){
								window.parent.location =cwpd_url;
							}else{
							  window.parent.location = store_index;
							}
						},1000);
