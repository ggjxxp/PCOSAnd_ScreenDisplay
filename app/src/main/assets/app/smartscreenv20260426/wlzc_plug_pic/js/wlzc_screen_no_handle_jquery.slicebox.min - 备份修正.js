(function(a, b, c) {
    ...
    b.slicebox.defaults = {
        ...
        slideshow: false,
        slideshowTime: 2e3,
		cyberwin_client_type:"computer",//未来之窗终端，computer ，计算机，
		autoplay:false,//自动播放
    ...
    b.Box3d.prototype = {
        _createBox: function(a) {
            ...
            this._addNavigation();
           // this._addOptions();
			this.wlzc_addOptions(a);
           // b('<div class="sb-shadow"/>').appendTo(this.wrapper);
			if(a.cyberwin_client_type=="screen_no_handle"){
			}else{
              b('<div class="sb-shadow"/>').appendTo(this.wrapper);
			}
			...
			if(a.autoplay==true){
			  this.isSlideshowActive = true;
              this._slideshow(a, true);
			  this.OptionPlay.removeClass("sb-nav-play").removeClass("rb-nav-pause");
			}
        ...
        _addOptions: function() {
            this.OptionPlay = b('<span class="sb-nav-play">自动播放</span>');
            this.OptionInfo = b('<span class="sb-nav-info">信息</span>');
		...
		wlzc_addOptions: function(wlzc_option) {
					console.log("未来之窗参数wlzc");
			console.log(a);
           this.OptionPlay = b('<span class="sb-nav-play">自动播放</span>');
            this.OptionInfo = b('<span class="sb-nav-info">信息</span>');
           // var a = b('<div class="sb-options">').append(this.OptionPlay).append(this.OptionInfo);
			var a = b('<div class="sb-options">');//.append(this.OptionPlay);//.append(this.OptionInfo);
			if(wlzc_option.cyberwin_client_type=="screen_no_handle"){
				//未来之窗无操作界面
				var a = b('<div class="sb-options">'); 
				 this.wrapper.append(a)
			}else{
				var a = b('<div class="sb-options">').append(this.OptionPlay).append(this.OptionInfo);
				 this.wrapper.append(a)
    ...
    b.Slice3d.prototype = {
        ...
        rotate: function(a, b, c, d, e, f) {
            var g = this,
            h, i = c.sequentialRotation ? c.sequentialFactor * b: 0;
            setTimeout(function() {
                if (a === "next") {
                    switch (g.side) {
                    case 1:
