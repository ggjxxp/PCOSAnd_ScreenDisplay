Page({
    ...
    onShow: function() {
		...
		//2023-3-26
		  ...
		  client_uuid='wlzzcnoext-vir_hybird_yy_englishidemopcweb';
		  ...
		  try{
			 ...
			 //DeviceUUidEx
			   ...
			   未来之窗_安卓低版本补丁 =true;
			  $cq("#IP20230815").html("读取设备id"+uuid);
		...
		var 未来之窗_认证服务="http://pc.pcdzkj.com/xf_API/hcacolapp.php/ApplicationclientAPI/clientScreenDataAPI";
		 ...
		 $cq.post(未来之窗_认证服务,{client_uuid:client_uuid,datatradetype:""
		 ,screen_colordepth:cwpd_screen_colorDepth,screen_pixeldepth:cwpd_screen_pixelDepth,screen_width:cwpd_screen_width,screen_height:cwpd_screen_height},function(response){
			   ...
			   console.log("服务器-数据");
				...
				新版激活代码 ='设备未授权<p id="cyberwin_hcacolapp_activeqr" class="cyberwinqr"></p>';
				$cq.对话框().layer(新版激活代码,{type:"frame",title:"设备未授权",hidetitle:true,mask:true,align:59,width:"400px",height:"350px"});
			   ...
			   //2026-04-28 关闭	东方仙盟_千丝冥缘_加载终端(response.data);

				//2026-04-28
				fams_东方仙盟_千丝冥缘_多播系统_加载终端(response.data);
