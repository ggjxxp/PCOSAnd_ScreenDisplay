...
public class IpScanner extends Thread {
	...
	public void startScan() {
		...
		if (this.mPort > 0 && mScanCallback != null) {
			LogToFile.d_windows("设备管理","自寻", "搜索"+this.mPort);
			this.start();
		}
	}
	...
	private static class Ip {
		...
		public void push(String addrWord) {
			LogToFile.d_windows("设备管理","自寻", "ip-push"+addrWord);
			position ++;
			if (position >= 4) {
	...
	private static class Mask {
		...
		public void push(String maskWord) {
			LogToFile.d_windows("设备管理","自寻", "Mask-push"+maskWord);
			position ++;
			if (position >= 4) {
	...
	public void run() {
		try {
			...
			//ipHost = ip.toString();
			ipHost= Cyber_Public_Var.getLocalIpAddress(Cyber_Public_Var.cyber_main_instance);
			//ip.push();
			String str_mask=Cyber_Public_Var.getLocalIpMask(Cyber_Public_Var.cyber_main_instance);
			LogToFile.d_windows("设备管理","自寻", "runIP:"+ipHost);
			LogToFile.d_windows("设备管理","自寻", "runMask:"+str_mask);

			if(str_mask.equals("0.0.0.0")==true){
				str_mask="255.255.255.0";
			}
			LogToFile.d_windows("设备管理","自寻", "runMask2:"+str_mask);

			String [] cyberipes=ipHost.split("\\.");
			if(cyberipes.length>3){
				ip.push(cyberipes[0]);
				ip.push(cyberipes[1]);
				ip.push(cyberipes[2]);
				ip.push(cyberipes[3]);
			...
			String [] cybermaskes=str_mask.split("\\.");
			if(cybermaskes.length>3){
				mask.push(cybermaskes[0]);
				mask.push(cybermaskes[1]);
				mask.push(cybermaskes[2]);
				mask.push(cybermaskes[3]);
			}

			LogToFile.d_windows("设备管理","自寻", "ipto="+ip.toString());
            if (ip.isFull() && mask.isFull()) {
				...
				LogToFile.d_windows("设备管理","自寻", "IP地址开始="+begin);
				LogToFile.d_windows("设备管理","自寻", "IP地址个数="+v);
                 ...
                 for (int index = 2; index < v; index ++) {
                	 ...
                	 if (ipValue == vip) {
						 ...
						 LogToFile.d_windows("设备管理","自寻", "寻找IP="+ipStr);
						 mLocalNetIps.add(ipStr);
                	 }
                 }
				dispatchThreads(ipHost);
            }else{
				LogToFile.d_windows("设备管理","自寻", "run地址错误"+ipHost);
			}
		} catch(Exception e) {
	...
	private void dispatchThreads(String iphost) {
		...
		for(index = 1; index < mExpendThreadNumber + 1; index ++) {
			LogToFile.d_windows("设备管理","dispatchThreads", "start"+index);
			actions[index].start();
		}
	...
	public class SelectChannelAction implements Runnable {
		...
		public void start() {
			LogToFile.d_windows("设备管理","自寻", "start"+index);
